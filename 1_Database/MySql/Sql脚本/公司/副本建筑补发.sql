...
CREATE TEMPORARY TABLE AfterCopyBuilding 
(
...
--  插入数据 --
INSERT INTO `player_dev`.`p_building` (`PlayerID`,`LandID`,`ID`,`BuildingID`,`BuildingLv`) SELECT f.id AS playerid,f.LandID,UUID(),f.BuildingID,f.BaseLv AS BuildingLv FROM 
(
SELECT e.* FROM (
--  查询出新规则下玩家应该有的建筑 --
