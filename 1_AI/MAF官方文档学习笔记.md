...
## 工作流-人工干预
    ...
    1、在工作流中启用请求和响应处理
        1、请求和响应通过称为 RequestPort 的特殊类型进行处理。
        2、RequestPort 是一个通信通道，允许执行程序发送请求和接收响应。
        3、当执行者向 RequestPort 发送消息时，请求端口将发出包含请求详细信息的 RequestInfoEvent 。
        4、外部系统可以侦听这些事件、处理请求并将响应发送回工作流。
        5、框架根据原始请求自动将响应路由回相应的执行程序
...
## 检查点和恢复
    ...
    2、从检查点恢复
        ...
        // 新开一个
        // var newRun = await InProcessExecution.ResumeStreamingAsync(newWorkflow, savedCheckpoint, checkpointManager).ConfigureAwait(false);
        await foreach (WorkflowEvent evt in run.WatchStreamAsync().ConfigureAwait(false))
        {
    ...
    3、保存执行器状态
        1、为了确保执行器的状态在检查点中被捕获，执行器必须重写 OnCheckpointingAsync 方法并将其状态保存到工作流上下文。
        2、为了确保从检查点恢复时正确还原状态，执行程序必须重写 OnCheckpointRestoredAsync 该方法并从工作流上下文加载其状态。
...
## LLM基础知识
    ...
    1、什么是 LLM？
        大型语言模型是在大量文本数据上训练的 神经网络 ，用于预测序列中的下一个标记。在核心上，LLM 只是两件事：
        1. 模型权重 - 在训练期间学习的数十亿个数值参数，用于对模型的知识进行编码。
        2. 体系结构代码 - 神经网络结构（通常是 转换器 ），用于运行权重以生成输出。
    2、Token
        LLM 不按字符逐个处理原始文本，而是使用Token。 tokenizer 将输入文本拆分为Token，这些Token是固定词汇中的子单词单位。 Token可以是完整单词（ "hello" ）、部分单词（ "un" +"believ" + "able" ）、单个字符或标点符号。
    3、如何训练LLM？
        阶段 1：预训练
            1、预训练是模型了解其大部分知识的地方。该模型接收来自 Internet 的大量文本（书籍、文章、代码、网站），并学习在给定所有先前令牌的情况下预测下一个令牌。 此阶段需要巨大的计算（数周或数月的数千个 GPU），并生成 一个基本模型。
            2、基本模型本质上是文本完成引擎。 出现提示时，它会根据训练数据中的模式生成合理的延续。但是，作为助手，基本模型并不特别有用 ， 它可能会以意外的方式继续文本， 生成有害内容或只是乱乱无章。 它不会可靠地按照说明进行操作。
        阶段 2：训练后
            训练后能将基础模型转变为实用的助手。 此阶段发生在多个阶段：
            1、监督式微调 (SFT) - 该模型在精选的优质对话数据集上进行训练：由人工编写的理想助理行为示例。 这些示例演示了模型如何 按照说明进行操作、回答问题有帮助、拒绝有害请求以及明确设置响应格式。 SFT 教授模型作为有用的助手的角色。
            2、基于人类反馈的强化学习（RLHF） - 在 SFT 之后，人类评分员比较模型响应对，并指明哪个更好。 此首选项数据训练奖励模型，然后用于 强化学习 ，以进一步调整 LLM 以响应人类喜欢的响应。 RLHF 可帮助模型了解在静态示例中难以捕获的微妙质量区别，例如简洁与彻底，或了解何时要求澄清。 这通常适用于 无法验证的域中，其中没有一个正确的答案，这与明确客观或根本真理（如算术）的问题不同。
        阶段 3：通过强化学习进行推理
            强化学习技术已经被应用于教导模型在生成最终答案之前逐步推理。 这些模型不立即做出响应，而是学会生成一系列思想——将问题分解为子步骤、探索替代项和验证其工作。
    4、推理的工作原理
        向 LLM 发送请求时，模型通过一个名为自动回归生成的过程一次生成一个令牌来生成其响应。
            1. 你的完整提示（系统消息、对话历史记录、用户输入）将转换为令牌并馈送到模型中。
            2. 该模型处理所有输入令牌，并对其词汇生成概率分布 ， 预测下一个令牌最有可能出现。
            3. 从该分布中选择令牌（受温度和其他采样参数影响）。
            4. 新令牌 将追加到完整序列中，整个更新的序列将反馈到模型中以生成下一个令牌。
            5. 这会重复，直到模型生成停止令牌或达到长度限制。
    5、面向开发人员的关键概念
        1、提示词角色：system、user等
        2、温度：控制结果的随机性

## 从 LLM 到代理

    1、它们采用标记化的消息序列，一次生成一个新令牌。 但是原始LLM 调用是 无状态 的 - 它没有内存，没有连接工具，也没有内置的方式来维护对话。 每个调用都从头开始。
    2、代理包装 LLM，其中包含生成真实应用程序所需的结构：持久标识、系统指令、工具、内存和协调所有应用程序的运行时循环。
...
## 添加技能
    ...
    1、工具和技能的区别
        主要区别在于范围之一：工具使代理能够执行一个操作;技能为代理提供处理整个域的知识和资源。他们是互补的。
        1、工具是单个可调用操作，一个具有名称、说明和参数架构的函数。当模型确定工具时，它会生成结构化调用，代理框架将执行该工具，结果返回到模型。 工具是代理行为的原子。
        2、技能是一系列领域专业知识。 它可以包括：
            说明 - 分步指导、决策规则和示例，告知代理如何处理。
            参考资料 - 策略文档、常见问题解答、模板和其他知识，代理可按需咨询。
            脚本 - 代理可以运行的可执行代码来执行特定操作（例如，根据策略规则检查费用数据的验证脚本）。

## 添加中间件

    中间件允许你使用可复用的行为包装代理的执行管道，从而在明确定义的节点拦截、检查和修改请求和响应。 将中间件视为代理周围的一系列同心层 ， 每个层都有机会在进入代理之前对输入执行操作，在到达调用方之前在输出上执行操作。

## 添加上下文提供程序

    它们是在每个代理调用之前和之后运行的组件，主动将相关信息注入上下文窗口，并选择性地从要存储的响应中提取状态以供将来使用。 它们提供代理内存、个性化设置和对外部知识的访问权限，而无需更改代理的说明或代码。

## Agent作为工具

    把agent转换为工具使用。weatherAgent.AsAIFunction()

## A2A

    每个 A2A 调用都是一个 HTTP 请求。
